...
const ReminderSettings = observer(() => {
  ...
  const handleTimeChange = (evt: ChangeEvent<HTMLInputElement>) => setNotificationTime(evt.target.value);

  const handleToggle = (evt: ChangeEvent<HTMLInputElement>) => setEnabled(evt.target.checked);
      ...
      <CardContent>
        ...
        {enabled && (
          <Box sx={{ mt: 2 }}>
            ...
            <List dense>
              {upcomingReminders?.map((reminder: IReminder) => (
                <ListItem
                  key={reminder.id}
                  secondaryAction={
                    <IconButton
