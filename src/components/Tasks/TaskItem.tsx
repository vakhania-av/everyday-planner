...
const TaskItem = observer(({ todo }: ITaskItemProps) => {
  const {
    editingTaskId,
    editText,
    editDeadline,
  ...
  const textItem = isEditing ? (
    ...
    >
      ...
      <DateTimePicker
        label="Deadline"
        value={editDeadline}
        onChange={(value: Dayjs | null) => setEditDeadline(value)}
        slotProps={{
          textField: {
            size: 'small',
            sx: { minWidth: 200 }
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap' }}>
          {todo.category && (
            <Chip
              ...
              color={categories.find((c) => c.id === todo.category)?.color as any || 'default'}
              size="small"
              sx={{ mr: 1, mb: 0.5 }}
            />
          )}
